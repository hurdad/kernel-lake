...
TEST(ArrowAdapter, RoundTripsPrimitiveTypes) {
  for (const DataType& type :
       {int32_type(), int64_type(), uint32_type(), uint64_type(), float32_type(), float64_type(),
        string_type(), date32_type(), timestamp_type(), boolean_type()}) {
...
TEST(ArrowAdapter, LargeStringMapsToStringType) {
  const DataType round_tripped = from_arrow_type(arrow::large_utf8(), true);
  EXPECT_EQ(round_tripped.id, TypeId::String);
}
