...
[[noreturn]] void fail(std::string_view text) {
  throw SqlError("invalid date literal '" + std::string(text) +
                 "': expected 'YYYY-MM-DD' with valid calendar values");
}

}  // namespace

std::int32_t parse_iso_date(std::string_view text) {
  if (text.size() != 10 || text[4] != '-' || text[7] != '-') fail(text);
  ...
  if (month < 1 || month > 12) fail(text);
  if (day < 1 || day > days_in_month(year, month)) fail(text);
