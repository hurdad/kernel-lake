...
void ArrowResultOperator::open(ExecutionContext& context) { child_->open(context); }

std::optional<DeviceBatch> ArrowResultOperator::next(ExecutionContext& context) {
  return child_->next(context);
}

void ArrowResultOperator::close(ExecutionContext& context) { child_->close(context); }
