...
std::vector<std::pair<std::string, std::string>> LogicalScan::explain_attributes() const {
  ...
  if (!pushable_predicates_.empty()) {
    ...
    out << "]";
    attrs.emplace_back("pushable_predicates", out.str());
  }
  return attrs;
}
...
void explain_text_recursive(const LogicalPlanNode& node, const std::string& prefix, std::ostringstream& out) {
  out << node.node_name() << "\n";
  ...
  for (std::size_t i = 0; i < children.size(); ++i) {
    out << attr_prefix << "└── ";
    explain_text_recursive(*children[i], attr_prefix, out);
  }
}
