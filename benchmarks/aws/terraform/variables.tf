...
variable "kernellake_docker_image" {
  description = "Docker image reference for the kernellake-server runtime-gpu image. Defaults to the image this repo's own CI (the docker-publish-gpu job in .github/workflows/ci.yml) already builds and pushes to GHCR on every push to main (confirmed via a real successful run -- includes the CUDA 13 Docker base image migration as of 2026-08-23, see docs/ARCHITECTURE.md's \"CUDA in docker/Dockerfile\" section) -- no manual build/push needed unless testing local changes, in which case build docker/Dockerfile's runtime-gpu target yourself and override this. Assumes the GHCR package is public (the default for a public source repo, which hurdad/kernel-lake is) -- if a pull fails with an auth error on the KernelLake host, either make the package public in the repo's Packages settings, or add a `docker login ghcr.io` step (with a PAT) to kernellake-host-init.sh. Also requires the host's own NVIDIA driver to be >=610.43.02 (CUDA 13.3's documented minimum) -- kernellake-host-init.sh checks and upgrades this automatically, see its own comment."
  type        = string
