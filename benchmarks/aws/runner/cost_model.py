...
class RunCost:
    ...
    def total_usd(self) -> float:
        return self.kernellake_usd + self.spark_usd + self.monitoring_usd + self.s3_storage_usd
...
def cost_per_completed_query(run_cost: RunCost, queries_completed: int, engine: str) -> float | None:
    ...
    if engine_cost is None:
        raise ValueError(f"Unknown engine '{engine}' -- expected 'kernellake' or 'pyspark'")
...
def cost_per_tb(run_cost: RunCost, bytes_processed: int, engine: str) -> float | None:
    ...
    if bytes_processed <= 0:
        return None
    engine_cost = {"kernellake": run_cost.kernellake_usd, "pyspark": run_cost.spark_usd}.get(engine)
    if engine_cost is None:
        raise ValueError(f"Unknown engine '{engine}' -- expected 'kernellake' or 'pyspark'")
...
def _main() -> int:
    ...
    parser.add_argument("--kernellake-instance-type", default="g6.8xlarge")
    ...
    print(f"Wrote {args.output}: total ${run_cost.total_usd:.2f} "
          f"(kernellake ${run_cost.kernellake_usd:.2f}, spark ${run_cost.spark_usd:.2f})")
    return 0
