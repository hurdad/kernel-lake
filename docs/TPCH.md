...
and **Q10** (a four-table `customer`/`orders`/`lineitem`/`nation`
`INNER JOIN` chain, a 7-column `GROUP BY` spanning two non-adjacent join
sources, `ORDER BY` plus `LIMIT`). KernelLake supports a chain of two or
...
and `nation` tables with TPC-H's column names and roughly TPC-H-shaped
value distributions (`nation` is the one exception: it's small enough that
the script hardcodes real `dbgen` reference data instead of sampling --
...
`customer-00000.parquet`, `nation-00000.parquet`, plus a `manifest.json`
...
150,000 `customer` rows; this generator targets the same
`lineitem`/`part`/`orders` row counts at the same scale factor
(`lineitem` split across `--files` Parquet files; `part`/`orders` always a
single file each -- `orders` gets exactly one row per distinct
`l_orderkey` generated, per TPC-H's own 1:N orders:lineitem relationship).
`customer` is a fixed 150,000 rows regardless of scale factor (matching
TPC-H's own SF1 customer count), also written as a single file. `nation`
is a fixed 25 rows regardless of scale factor (real TPC-H's own nation
count, and real `dbgen` reference data -- names and region keys -- rather
than sampled values), also a single file.

## 2. Query

The queries live in version-controlled files, `benchmarks/tpch/queries/
q01.sql`, `q03.sql`, `q06.sql`, `q10.sql`, `q12.sql`, `q14.sql`, `q19.sql`,
each with a header comment documenting its specific deviations from
canonical TPC-H syntax (`FROM lineitem` -> `FROM read_parquet('{data}')`,
no `INTERVAL` arithmetic). Q1/Q6 need only `{data}` substituted with your
...
your `orders` and `customer` globs; Q10 needs `{orders_data}`,
`{customer_data}`, and `{nation_data}` substituted with your `orders`,
`customer`, and `nation` globs. `{data}` must always be a
...
# --customer-data, and --nation-data (none covered by --query all, which
# only passes --data):
python3 tools/validate_tpch.py \
  --kernellake build/gpu-dev/src/cli/kernellake \
...
compression, 1,000,000-row row groups). Q19, Q12, Q14, Q3, and Q10 have
each been verified at SF0.01 on both the CPU and GPU backends, exact match
against DuckDB (Q3 including its 3-way join, `ORDER BY revenue DESC,
o_orderdate` multi-key sort, and `LIMIT 10`; Q10 including its 4-way join,
7-column `GROUP BY` spanning two non-adjacent join sources, and `LIMIT
20`) -- Q19/Q12/Q14/Q3 also cross-validated against PySpark and DuckDB via
...
`--cost-per-hour`-based cost-per-TB comparison); Q10 has not been wired
into `benchmark_three_way.py` yet (see `docs/ROADMAP.md`'s "Not yet
started").
...
for Q12, `--orders-data` and `--customer-data` for Q3, or `--orders-data`,
`--customer-data`, and `--nation-data` for Q10 (or any combination for a
