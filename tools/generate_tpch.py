...
record every one:
  ...
  - Row content (prices, dates, flags, brands, containers, priorities,
    ...
    relationship; `customer` is a fixed `CUSTOMER_ROWS` regardless of SF,
    matching `orders`' own fixed `o_custkey` range -- see below).
  - `l_partkey` is drawn uniformly from `[1, total_part_rows]`, and
    `o_custkey`/`c_custkey` both use a fixed `[1, CUSTOMER_ROWS]` range
    regardless of scale factor (real TPC-H's `customer` count scales with
    SF; this generator's doesn't) -- neither is generated by dbgen's
    actual selection distribution, but both are sufficient for every join
    in this table's row range to find a real matching row (no dangling
    foreign keys), which is what actually matters for validating query
    correctness.
